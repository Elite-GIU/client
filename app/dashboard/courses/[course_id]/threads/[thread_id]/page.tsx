...
function ThreadDetailPage() {
  ...
  useEffect(() => {
    const fetchThreadAndMessages = async () => {
      ...
      try {
        ...
        const threadResponse = await fetch(`/api/dashboard/courses/${course_id}/threads/${thread_id}`, {
          headers: { Authorization: `Bearer ${token}` },
        });
        if (!threadResponse.ok) {
          throw new Error(`Error fetching thread: ${threadResponse.statusText}`);
        }
        const threadData = await threadResponse.json();
        setThread(threadData);

        // Fetch messages
        const messagesResponse = await fetch(`/api/dashboard/courses/${course_id}/threads/${thread_id}/messages`, {
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const updateThread = async (updatedTitle: string, updatedDescription: string) => {
    const token = Cookies.get('Token');
    if (!token) {
    ...
    try {
      const response = await fetch(`/api/dashboard/courses/${course_id}/threads/${thread_id}`, {
        method: 'PUT',
        headers: {
      ...
      setThread((prevThread) =>
        prevThread ? { ...prevThread, title: updatedTitle, description: updatedDescription } : null
      );
  ...
  const sendMessage = async (message: string, parentId?: string) => {
    ...
    try {
      const response = await fetch(
        `/api/dashboard/courses/${course_id}/threads/${thread_id}/messages${parentId ? `/${parentId}/` : ''}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
